...
char            index_overdrive[9];      //hay 8 tipos de datos
...
void  ini_I2C_Data(char            flag_var_randon)
{
  ...
  EEROM_Dosifi.KW_Disponible_dia_anterior=0;
  EEROM_Dosifi.KW_Dofi_Inicio_ref=0;
  EEROM_Dosifi.KW_Consumo_total=0;
  EEROM_Dosifi.Indicador_ref_hora=false;
  ...
  //No se colocan en Parametros_CFG, porque se modifican mas a menudo.
  store_flashESP32(Add_flash(TYPE_ID_CFG_MODO),Data_size(TYPE_ID_CFG_MODO),(char*)&ESP_WIFI_CFG_MODO);
  
  temp=28;
  store_flashESP32(Add_flash(TYPE_FECHA_CORTE_KW),Data_size(TYPE_FECHA_CORTE_KW),(char*)&temp);  
  temp=1;
  store_flashESP32(Add_flash(TYPE_FECHA_CORTE_PR),Data_size(TYPE_FECHA_CORTE_PR),(char*)&temp);  
  
  END_ADD=TAM_PARAMETROS_CFG+TAM_ID_CFG_MODO+TAM_FECHA_FAB+TAM_FECHA_CORTE_KW+TAM_FECHA_CORTE_PR+3;
...
void  store_flashI2C(int Data_type,char* Info)
{
  ...
  //1. Escribo en direccion actual
  I2CMEN.write(Add_flash(Data_type),(byte*)Info,Data_size(Data_type));
  
  //2. Aumento uno para no sobre escribir, y grabo el index
  //TYPE_FLAGS, no maneja buffer
  if(Data_type>TYPE_FLAGS)
  {
    ...
    if(Data_index==DATA_INDEX_OVERDRIVE)
    {
      ...
      #endif
      Data_index=0;
      index_overdrive[Data_type]=1; //indica que index es 0, pero vengo del tope.
    }
    else
    {
      index_overdrive[Data_type]=0; //nodo normal
    }
...
void  read_flashI2C(int Data_type,char* Info)
{
  ...
  //1. Decremento index, y actualizo el index.
  if(Data_type>TYPE_FLAGS)
  {
    if( index_overdrive[Data_type]==1)
    {
      //index=tope-1
      I2CMEN.writeInt(sizeof(int)*Data_type, DATA_INDEX_OVERDRIVE-1);
    }
    else
    {
      Data_index=I2CMEN.readInt(sizeof(int)*Data_type);
      //Si es caso inicial index 0, index no se decrementa.
      if(Data_index>0) 
        Data_index--;
      I2CMEN.writeInt(sizeof(int)*Data_type,Data_index);
    }
  }
  //2. Leo dato con index -1.
  I2CMEN.read(Add_flash(Data_type),(byte*)Info ,Data_size(Data_type));  


  //3. Me posicion en index denuevo, para aceptar un dato nuevo.
  if(Data_type>TYPE_FLAGS)
  {
   Data_index=I2CMEN.readInt(sizeof(int)*Data_type);
    Data_index++;
...
//sizeof(int)*1   xxxxxxxxxx, (TYPE_FLAGS) no usa index 
...
int Add_flash(int type)
{
  ...
  switch (type) 
  {
    ...
    break;
    case TYPE_FECHA_FAB:
        result=TAM_PARAMETROS_CFG+TAM_ID_CFG_MODO+1;
    break;
    case TYPE_FECHA_CORTE_KW:
        result=TAM_PARAMETROS_CFG+TAM_ID_CFG_MODO+TAM_FECHA_FAB+1;
    break;
    case TYPE_FECHA_CORTE_PR:
        result=TAM_PARAMETROS_CFG+TAM_ID_CFG_MODO+TAM_FECHA_FAB+TAM_FECHA_CORTE_KW+1;
    break;
...
int Data_size(int type)
{
  ...
  switch (type) 
  {
    ...
    break;     
    case TYPE_FECHA_FAB:
       result=  TAM_FECHA_FAB;
    break;
    case TYPE_FECHA_CORTE_KW:
       result=  TAM_FECHA_CORTE_KW;
    break;
    case TYPE_FECHA_CORTE_PR:
       result=  TAM_FECHA_CORTE_PR;
    break;      
    default:
     result=0;
    break;
